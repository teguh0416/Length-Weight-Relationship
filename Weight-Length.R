...
rru1 <- data.frame(
  length1 = seq(min(rru$length), max(rru$length), by = sl_kelas)
) 

rru2 <- rru1 %>% mutate(log_length = log(rru1$length)) %>% 
        mutate(w_est = exp(lm$coefficients[1]) * (rru1$length ^ lm$coefficients[2]), row.names = NULL)
...
plot1 <- ggplot(data=rru,aes(x = length, y = weight)) + geom_point(aes(color="Estimasi")) +
  geom_point(data=rru2,aes(x = length1, y = w_est, color="Pengukuran"))+
